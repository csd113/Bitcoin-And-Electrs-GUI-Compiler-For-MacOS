...
def show_message(kind, title, msg):
    """Thread-safe wrapper for messagebox dialogs.
    
    Schedules the dialog on the main thread via root.after() so it is safe
    to call from any background thread.  'kind' must be a valid messagebox
    function name, e.g. 'showerror', 'showinfo', 'showwarning'.
    """
    root.after(0, lambda: getattr(messagebox, kind)(title, msg))

def ask_yes_no_threadsafe(title, msg):
    """Thread-safe wrapper for messagebox.askyesno.
    
    Schedules the dialog on the main thread and blocks the calling thread
    until the user responds, then returns the boolean result.
    """
    result = [None]
    ...
    def task():
        try:
            ...
            if not BREW:
                ...
                show_message("showerror", "Missing Dependency", "Homebrew not found! Please install from https://brew.sh")
                return
            ...
            if missing:
                ...
                try:
                    ...
                    install_deps = ask_yes_no_threadsafe("Install Missing Dependencies", message)
                except Exception as e:
...
def refresh_bitcoin_versions():
    ...
    if versions:
        ...
        show_message("showwarning", "Network Error", "Could not fetch Bitcoin versions. Check your internet connection.")
...
def refresh_electrs_versions():
    ...
    if versions:
        ...
        show_message("showwarning", "Network Error", "Could not fetch Electrs versions. Check your internet connection.")
        ...
        try:
            ...
            if target in ["Bitcoin", "Both"]:
                if not bitcoin_ver or bitcoin_ver == "Loading...":
                    show_message("showerror", "Error", "Please wait for Bitcoin versions to load, or click Refresh")
                    return
            
            if target in ["Electrs", "Both"]:
                if not electrs_ver or electrs_ver == "Loading...":
                    show_message("showerror", "Error", "Please wait for Electrs versions to load, or click Refresh")
                    return
